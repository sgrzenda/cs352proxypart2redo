 ...
 * its not in the list
 */
void add_members(linkstate_node* list, uint8_t* sourceMac) {
   // int count = 1;
    ...
    //    printf("count is %d\n", count);
    //    count++;
        add_member_connect(ptr, sourceMac);
        ptr = ptr->next;
    }
}
 ...
 */
void add_member_connect(linkstate_node* recv_node, uint8_t* sourceMac) {
    //pthread_mutex_lock(&deleterlock);
   // printf("Got to add_member_connect part2\n");
    printf("This is the recv_node->local macaddr: ");
    int j;
    for (j = 0; j < 6; j++) {
        printf("%x:",recv_node->local.macaddr[j]);
    }
    printf("\nThis is the recv_node->remote macaddr: ");
    for (j = 0; j < 6; j++) {
    ...
    if (pkt_sockfd == NULL) {
        ...
        if (comp_mac_addrs(recv_node->local.macaddr, my_proxy_info->macaddr) == 0) {
            printf("But it was me\n");
            linkstate_node * finder = in_member_list(&(recv_node->local), & (recv_node->remote));
            if (finder == NULL) {
                finder = in_member_list(&(recv_node->remote), & (recv_node->local));
            }
            if (finder != NULL) {
       ...
       // printf("This is the remote IP %s\n", inet_ntoa(*(struct in_addr*)&recv_node->remote.ip));
        
       // printf("This is the local IP %s\n", inet_ntoa(*(struct in_addr*)&recv_node->local.ip));
       ...
       // printf("Portno is %d\n", portno);
        ...
        struct proxy_socketfd* sfd = find_socketfd(recv_node->local.macaddr);
        ...
        if (comp_mac_addrs(recv_node->remote.macaddr, my_proxy_info->macaddr) == 0) {
            ...
            printf("The remote is not me\n");
            struct proxy_socketfd* pkt_sockfd2 = find_socketfd(recv_node->remote.macaddr);
            ...
            if (pkt_sockfd2 == NULL) {
                ...
                printf("Portno is %d\n", portno);
                int sock_fd = connecttohost32(portno,ip);
                printf("Got back from connect to host\n");
                if (sock_fd < 0) {
                    perror("Could not connect to host.");
                    pthread_mutex_unlock(&deleterlock);
                    return;
                ...
                addsockfd(recv_node->remote.macaddr,sock_fd);
                pthread_t startListen;
                ...
                linkstate_node* temp2 = mem_list->head;
                printf("\n\n");
                while (temp2 != NULL) {
                    printf("This is in the list from add_mem %s\n", inet_ntoa(*(struct in_addr*)&temp2->remote.ip));
                    temp2 = temp2->next;
                }
                printf("\n\n");
            ...
            printf("This is the sourceMac macaddr: ");
            int j;
            for (j = 0; j < 6; j++) {
                printf("%x:",sourceMac[j]);
            }
            printf("\nThis is the recv_node->local macaddr: ");
            for (j = 0; j < 6; j++) {
                printf("%x:",recv_node->local.macaddr[j]);
            }
            printf("\n");
            if (comp_mac_addrs(sourceMac, recv_node->local.macaddr) == 0) {
                linkstate_node * finder = in_member_list(&(recv_node->local), & (recv_node->remote));
                if (finder == NULL) {
                    finder = in_member_list(&(recv_node->remote), & (recv_node->local));
                }
...
void * sendPackets() {
    while(1) {
        sleep(linkPeriod);
        printf("Done sleeping, transmitting membership list linkstate pkt\n");
...
void *TCPHandle (void* fd) {
    ...
    while (1) {
       ...
       /* linkstate_node* temp = mem_list->head;
        while (temp != NULL) {
            printf("\n\n\nThis is in the list %s\n", inet_ntoa(*(struct in_addr*)&temp->remote.ip));
            temp = temp->next;
        }
        printf("\n\n");
        */
	       ...
	       // printf("\n\n");
            ...
            if (pkt_sockfd == NULL) {   // The initial receive, not in hashtable yet
                ...
                //    printf("\n\n\nAttempting to add recieved list to my list\n\n\n");
                    ...
                    add_members(pkt->list, pkt->source.macaddr);
                }
                ...
                //    printf("\n\n\nGot the original packet sent from connectToHost\n\n\n");
	             ...
	             //   printf("This is the remote IP %s\n", inet_ntoa(*(struct in_addr*)&client_node2->remote.ip));
                   ...
                   /* linkstate_node* temp2 = mem_list->head;
                    while (temp2 != NULL) {
                        printf("\n\n\nThis is in the list after the next send %s\n", inet_ntoa(*(struct in_addr*)&temp2->remote.ip));
                        temp2 = temp2->next;
                    }
                    printf("\n\n");
                    */
                    ...
                    head2->length = 14 + 36*mem_list->size; // Change this based on the membership list.

                    char buf2[4+head2->length];
                    //bzero(buf2, 54);
                    ...
                    send(tcp_fd, buf2,4+head2->length,0);
                } else {
